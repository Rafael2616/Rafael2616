<p align="center">
  <img src="https://avatars.githubusercontent.com/u/93414086?v=4"
       alt="Banner - Rafael"
       width="100%"
       style="border-radius: 12px;" />
</p>

<h1 align="center">👋 Olá! Eu sou o Rafael</h1>

<p align="center">
  <b>Engenheiro de Software • Android Dev • Kotlin Multiplatform</b><br/>
  Construindo experiências modernas com Kotlin, Compose e arquitetura limpa.
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Kotlin-7F52FF?style=for-the-badge&logo=kotlin&logoColor=white"/>
  <img src="https://img.shields.io/badge/Compose-4285F4?style=for-the-badge&logo=jetpackcompose&logoColor=white"/>
  <img src="https://img.shields.io/badge/KMP-000000?style=for-the-badge&logo=kotlin&logoColor=white"/>
  <img src="https://img.shields.io/badge/Android-3DDC84?style=for-the-badge&logo=android&logoColor=white"/>
</p>

---

## 🚀 Sobre mim

Sou desenvolvedor focado em **Android com Kotlin** e **Jetpack Compose**, sempre buscando criar interfaces fluidas, acessíveis e escaláveis.  
Atualmente também trabalho com **Kotlin Multiplatform (KMP)** para entregar soluções multiplataforma com performance nativa.

Tenho experiência com:
- Google Maps (Android & iOS)
- Processamento e otimização de imagens
- Firebase, autenticação e notificações
- Acesso a arquivos e permissões (Android/iOS)
- Compose Desktop e WASM/JS
- Arquiteturas modernas (MVVM, MVI, Clean Architecture)

Adoro resolver problemas complexos, refinar desempenho e transformar boas ideias em produtos reais.

---

## 🛠️ Tecnologias & Ferramentas

<div align="center">

### 👨‍💻 Linguagens  
<img src="https://skillicons.dev/icons?i=kotlin,java,python"/>

### 📱 Mobile & Multiplataforma  
<img src="https://skillicons.dev/icons?i=androidstudio"/>
<br>
<b>Kotlin Multiplatform • Jetpack Compose • Compose Multiplatform</b>

### ☁️ Backend & Cloud  
<img src="https://skillicons.dev/icons?i=firebase,aws"/>

### 🧰 Ferramentas  
<img src="https://skillicons.dev/icons?i=git,github,gradle"/>

</div>

## 📌 Projetos em Destaque

### 🔹 InclusiMap — Acessibilidade inteligente para cadeirantes  
App voltado para indicar locais acessíveis usando geolocalização, upload de imagens, Google Maps e acessibilidade real.  
🔗 **Repositório:** https://github.com/Rafael2616/InlcusiMap

### 🔹 Agropec 2026 — Eventos e notificações avançadas  
Aplicativo oficial da maior feira agropecuária da região Norte.  
🔗 **Repositório:** https://github.com/Rafael2616/Agropec2026

### 🔹 Tic Tac Toe — Compose Multiplatform  
Jogo da velha com Material You, título customizado e suporte para Desktop + Web.  
🔗 **Repositório:** https://github.com/Rafael2616/TocTacToeYou

---

## 📈 Estatísticas do GitHub

<div align="center">
  
![GitHub Stats](https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Rafael2616&theme=tokyonight)

![Languages](https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=Rafael2616&theme=tokyonight)

</div>

---

## 📫 Como me encontrar

<div align="center">

📧 **Email:** *rafaelmoura135790@gmail.com*
💼 **LinkedIn:** *https://www.linkedin.com/in/rafael-de-moura-nascimento-68133a265*  

</div>

---

<p align="center">
  Feito com ❤️ por Rafael.
</p>>
